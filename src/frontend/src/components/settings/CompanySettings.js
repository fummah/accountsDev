...
import {
  ...
  InputNumber, Spin, Divider, Typography
} from 'antd';
import {
    ...
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      ...
      <Form form={form} layout="vertical">
        <div style={{ display: 'flex', gap: 0, marginBottom: 16, borderBottom: '2px solid #e8e8e8' }}>
          {[
            { key: 'company', icon: <BuildOutlined />, label: 'Company Info' },
            { key: 'branding', icon: <PictureOutlined />, label: 'Branding' },
            { key: 'currency', icon: <GlobalOutlined />, label: 'Currency & Tax' },
            { key: 'banking', icon: <BankOutlined />, label: 'Banking' },
          ].map(t => (
            <div key={t.key} onClick={() => setActiveTab(t.key)}
              style={{ padding: '10px 20px', cursor: 'pointer', fontWeight: activeTab === t.key ? 600 : 400, color: activeTab === t.key ? '#1890ff' : '#555', borderBottom: activeTab === t.key ? '2px solid #1890ff' : '2px solid transparent', marginBottom: -2, transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: 6, background: activeTab === t.key ? '#fff' : 'transparent' }}>
              {t.icon} {t.label}
            </div>
          ))}
        </div>
        ...
        {activeTab === 'company' && (
          <Card size="small">
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <Form.Item name="state" label="State" rules={[{ required: true, message: 'Required' }]} style={{ flex: 1 }}>
                <Input placeholder="NY" />
              </Form.Item>
              <Form.Item name="postal_code" label="ZIP / Postal Code" rules={[{ required: true, message: 'Required' }]} style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <Form.Item name="phone" label="Phone" style={{ flex: 1 }}>
                <Input placeholder="+1 (555) 123-4567" onChange={e => form.setFieldsValue({ phone: phoneInputHandler(e.target.value) })} />
              </Form.Item>
