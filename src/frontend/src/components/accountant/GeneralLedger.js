...
const GeneralLedger = () => {
  ...
  const handlePrint = () => {
    ...
    const rowsHtml = withRunningBalance.map(r => `<tr><td>${moment(r.date).format('MM/DD/YYYY')}</td><td>${r.description || ''}</td><td>${r.reference || ''}</td><td style="text-align:right">${Number(r.debit) ? fmt(r.debit) : ''}</td><td style="text-align:right">${Number(r.credit) ? fmt(r.credit) : ''}</td><td style="text-align:right">${fmt(r.balance)}</td></tr>`).join('');
