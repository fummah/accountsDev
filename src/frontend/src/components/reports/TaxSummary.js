...
const TaxSummary = () => {
  ...
  const handleExport = () => {
    try {
      ...
      lines.push('Name,Rate,Output Tax,Input Tax');
      ...
      lines.push(`Total Tax Collected,${summary.totalVatCollected.toFixed(2)}`);
      lines.push(`Total Tax Paid,${summary.totalVatPaid.toFixed(2)}`);
      lines.push(`Net Tax,${(summary.totalVatCollected - summary.totalVatPaid).toFixed(2)}`);
  ...
  const vatColumns = [
    { title: 'Tax Rate', key: 'name', render: (_, r) => r.vat_name || `Tax ${r.vat || r.vat_percentage || 0}%` },
    ...
    { title: 'Output Tax (Collected)', key: 'output', width: 160, render: (_, r) => `${cSym} ${(Number(r.total_vat_sum || r.output_vat || 0)).toFixed(2)}` },
    ...
    <div className="gx-p-4">
      ...
      >
        <Row gutter={24} style={{ marginBottom: 24 }}>
          ...
          <Col span={6}>
            <Statistic title="Net Tax Payable" value={netVat} precision={2} prefix={cSym} valueStyle={{ color: netVat >= 0 ? '#cf1322' : '#3f8600' }} />
          </Col>
        </Row>
